	...
	SubShader
	{
		...
		Pass
		{
						...
						, bool ase_vface : SV_IsFrontFace ) : SV_Target
			{
				...
				float2 break26_g752 = uv_BaseMap;
				float GlobalTilingX11 = ( _GlobalXYTilingXYZWOffsetXY.x - 1.0 );
				float GlobalTilingY8 = ( _GlobalXYTilingXYZWOffsetXY.y - 1.0 );
				float2 appendResult14_g752 = (float2(( break26_g752.x * GlobalTilingX11 ) , ( break26_g752.y * GlobalTilingY8 )));
				float GlobalOffsetX10 = _GlobalXYTilingXYZWOffsetXY.z;
				float GlobalOffsetY9 = _GlobalXYTilingXYZWOffsetXY.w;
				float2 appendResult13_g752 = (float2(( break26_g752.x + GlobalOffsetX10 ) , ( break26_g752.y + GlobalOffsetY9 )));
				...
				float4 tex2DNode7_g751 = tex2D( _BaseMap, ( ( appendResult14_g752 + appendResult13_g752 ) + Parallaxe374 ) );
				float4 lerpResult53_g751 = lerp( _BaseColor , ( ( _BaseColor * tex2DNode7_g751 ) * _BaseColor.a ) , _BaseColor.a);
				float clampResult27_g751 = clamp( _Saturation , -1.0 , 100.0 );
				float3 desaturateInitialColor29_g751 = lerpResult53_g751.rgb;
				float desaturateDot29_g751 = dot( desaturateInitialColor29_g751, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar29_g751 = lerp( desaturateInitialColor29_g751, desaturateDot29_g751.xxx, -clampResult27_g751 );
				float4 temp_output_462_0 = CalculateContrast(_Brightness,float4( desaturateVar29_g751 , 0.0 ));
				...
				float2 texCoord394 = IN.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
				...
				float3 staticSwitch393 = IN.ase_texcoord9.xyz;
				...
				#elif defined(_CHANNELSELECTIONMASK1_GREEN)
				float staticSwitch456 = triplanar400.g;
				#elif defined(_CHANNELSELECTIONMASK1_BLUE)
				float staticSwitch456 = triplanar400.b;
				#elif defined(_CHANNELSELECTIONMASK1_ALPHA)
				float staticSwitch456 = triplanar400.a;
				#else
				float staticSwitch456 = triplanar400.x;
				#endif
				float4 temp_cast_8 = ((( _InvertMask )?( ( 1.0 - (( _EnableTriplanarProjection )?( staticSwitch456 ):( staticSwitch457 )) ) ):( (( _EnableTriplanarProjection )?( staticSwitch456 ):( staticSwitch457 )) ))).xxxx;
				float4 clampResult414 = clamp( ( CalculateContrast(( _ContrastDetailMap + 1.0 ),temp_cast_8) + -_SpreadDetailMap ) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float MaskIntensity412 = _IntensityMask;
				float4 Mask158 = (( _EnableDetailMask )?( ( clampResult414 * MaskIntensity412 ) ):( temp_cast_6 ));
				float4 lerpResult343 = lerp( temp_output_462_0 , AlbedoDetail153 , Mask158);
				float4 blendOpSrc15_g715 = temp_output_462_0;
				...
				float2 break26_g754 = uv_BumpMap;
				float2 appendResult14_g754 = (float2(( break26_g754.x * GlobalTilingX11 ) , ( break26_g754.y * GlobalTilingY8 )));
				float2 appendResult13_g754 = (float2(( break26_g754.x + GlobalOffsetX10 ) , ( break26_g754.y + GlobalOffsetY9 )));
				float3 unpack4_g753 = UnpackNormalScale( tex2D( _BumpMap, ( ( appendResult14_g754 + appendResult13_g754 ) + Parallaxe374 ) ), _BumpScale );
				unpack4_g753.z = lerp( 1, unpack4_g753.z, saturate(_BumpScale) );
				float3 tex2DNode4_g753 = unpack4_g753;
				float3 temp_output_463_0 = tex2DNode4_g753;
				...
				float3 unpack4_g750 = UnpackNormalScale( tex2D( _DetailNormalMap, uv_DetailNormalMap ), _DetailNormalMapScale );
				unpack4_g750.z = lerp( 1, unpack4_g750.z, saturate(_DetailNormalMapScale) );
				float3 NormalDetail155 = unpack4_g750;
				float3 lerpResult137 = lerp( temp_output_463_0 , NormalDetail155 , Mask158.rgb);
				float3 lerpResult205 = lerp( temp_output_463_0 , BlendNormal( temp_output_463_0 , NormalDetail155 ) , Mask158.rgb);
				...
				float2 break26_g758 = uv_EmissionMap;
				float2 appendResult14_g758 = (float2(( break26_g758.x * GlobalTilingX11 ) , ( break26_g758.y * GlobalTilingY8 )));
				float2 appendResult13_g758 = (float2(( break26_g758.x + GlobalOffsetX10 ) , ( break26_g758.y + GlobalOffsetY9 )));
				float4 temp_output_5_0_g757 = ( _EmissionColor * tex2D( _EmissionMap, ( ( appendResult14_g758 + appendResult13_g758 ) + Parallaxe374 ) ) );
				float4 temp_output_465_0 = ( temp_output_5_0_g757 * _EmissionIntensity );
				float4 temp_cast_18 = (0.0).xxxx;
				float4 lerpResult190 = lerp( temp_output_465_0 , temp_cast_18 , Mask158);
				float4 Emission110 = (( _UseEmissionFromMainProperties )?( temp_output_465_0 ):( lerpResult190 ));
				...
				float switchResult34_g745 = (((ase_vface>0)?(_Smoothness):(_SmoothnessBack)));
				float BaseColorAlpha46 = (( _InvertABaseColor1 )?( ( 1.0 - tex2DNode7_g751.a ) ):( tex2DNode7_g751.a ));
				#if defined(_SOURCE_METALLICALPHA)
				float staticSwitch23_g745 = ( tex2DNode3_g745.a * switchResult34_g745 );
				#elif defined(_SOURCE_BASECOLORALPHA)
				float staticSwitch23_g745 = ( switchResult34_g745 * BaseColorAlpha46 );
				#else
				float staticSwitch23_g745 = ( tex2DNode3_g745.a * switchResult34_g745 );
				#endif
				...
				float2 break26_g756 = uv_OcclusionMap;
				...
				float blendOpSrc2_g755 = tex2D( _OcclusionMap, ( ( appendResult14_g756 + appendResult13_g756 ) + Parallaxe374 ) ).r;
				float blendOpDest2_g755 = ( 1.0 - _OcclusionStrength );
				float temp_output_464_0 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g755 ) * ( 1.0 - blendOpDest2_g755 ) ) ));
				float lerpResult185 = lerp( temp_output_464_0 , 1.0 , Mask158.r);
				float AmbientOcclusion94 = (( _UseAoFromMainProperties )?( temp_output_464_0 ):( lerpResult185 ));
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN , bool ase_vface : SV_IsFrontFace ) : SV_TARGET
			{
				...
				float4 AlbedoDetail153 = CalculateContrast(_DetailBrightness,float4( desaturateVar29_g691 , 0.0 ));
				float4 temp_cast_6 = (0.0).xxxx;
				float2 texCoord394 = IN.ase_texcoord4.xy * float2( 1,1 ) + float2( 0,0 );
				float4 tex2DNode399 = tex2D( _DetailMask, texCoord394 );
				#if defined(_CHANNELSELECTIONMASK1_RED)
				float staticSwitch457 = tex2DNode399.r;
				#elif defined(_CHANNELSELECTIONMASK1_GREEN)
				...
				float4 lerpResult190 = lerp( temp_output_465_0 , temp_cast_16 , Mask158);
				float4 Emission110 = (( _UseEmissionFromMainProperties )?( temp_output_465_0 ):( lerpResult190 ));
		...
		Pass
		{
						...
						, bool ase_vface : SV_IsFrontFace )
			{
				...
				float2 texCoord394 = IN.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float4 tex2DNode399 = tex2D( _DetailMask, texCoord394 );
				#if defined(_CHANNELSELECTIONMASK1_RED)
				float staticSwitch457 = tex2DNode399.r;
				#elif defined(_CHANNELSELECTIONMASK1_GREEN)
				...
				float _TriplanarFalloff_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardCutout,_TriplanarFalloff);
				#if defined(_TRIPLANARSPACEPROJECTION_OBJECTSPACE)
				float3 staticSwitch393 = IN.ase_texcoord7.xyz;
				#elif defined(_TRIPLANARSPACEPROJECTION_WORLDSPACE)
				float3 staticSwitch393 = WorldPosition;
				#else
				float3 staticSwitch393 = IN.ase_texcoord7.xyz;
				...
				float staticSwitch456 = triplanar400.x;
				#endif
				float4 temp_cast_4 = ((( _InvertMask )?( ( 1.0 - (( _EnableTriplanarProjection )?( staticSwitch456 ):( staticSwitch457 )) ) ):( (( _EnableTriplanarProjection )?( staticSwitch456 ):( staticSwitch457 )) ))).xxxx;
				float4 clampResult414 = clamp( ( CalculateContrast(( _ContrastDetailMap + 1.0 ),temp_cast_4) + -_SpreadDetailMap ) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float MaskIntensity412 = _IntensityMask;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN , bool ase_vface : SV_IsFrontFace) : SV_TARGET
			{
				...
				surfaceDescription.Alpha = Cutout362;
				surfaceDescription.AlphaClipThreshold = CutOff430;
